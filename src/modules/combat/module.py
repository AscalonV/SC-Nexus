...
class CombatModule(BaseModule):
    ...
    def __init__(self, app, config: AppConfig):
        ...
        self.progress_popup: Optional[tk.Toplevel] = None
        self.progress_label_var = tk.StringVar(value="")
        self.progress_file_var = tk.StringVar(value="")
        self._progress_popup_size = (450, 220)
        ...
        self._bind_app_visibility_events()
    ...
    def on_show(self):
        if not self._initial_popup_shown:
            self._initial_popup_pending = True
        self._reload_fights()
    ...
    def _analyze_all_fights(self) -> None:
        ...
        if self.progress:
            self.progress.configure(mode="determinate", maximum=len(names_list), value=0)
        self.progress_label_var.set("Analyzing all fights...")
        self.progress_file_var.set("Preparing API checks")
    ...
    def _reload_fights(self, forced_logs: Optional[List[Path]] = None):
        ...
        def worker():
            if self._cancel_loading: return
    ...
    def _apply_fight_results(self, fights: List[parser.Fight]):
        ...
        # Force UI update to ensure everything is rendered before closing the popup
        self.frame.update()
        
        # Save cache in background to avoid UI freeze
        self._save_fight_cache()
        
        # Small delay to let the user see the "Ready" state or just ensure responsiveness
        self.frame.after(500, lambda: self._set_busy(False, "Ready"))

    def _init_progress(self, total: int) -> None:
        self._ensure_progress_popup()
        self._maybe_show_initial_overlay()
        if self.progress:
            self.progress.configure(mode="determinate", maximum=total, value=0)
        self.progress_label_var.set(f"Loading fights... 0/{total}")
        self.progress_file_var.set("")
        if self.file_progress:
            self.file_progress.configure(mode="determinate", maximum=1, value=0)
        self.current_file_path = None

    def _maybe_show_initial_overlay(self) -> None:
        if not self._initial_popup_pending or self._initial_popup_shown:
            return
        # Ensure the progress popup is created before showing the overlay
        if not (self.progress_popup and tk.Toplevel.winfo_exists(self.progress_popup)):
            self.frame.after(40, self._maybe_show_initial_overlay)
            return
        # Give the popup a tick to draw, then show the overlay behind it
        self.frame.after(0, self._show_initial_popup)
    ...
    def _update_progress(self, value: int, total: int, file_path: Path) -> None:
        ...
        self.progress_label_var.set(f"Loading fights... {value}/{total}")
        self.current_file_path = file_path
        # Do not overwrite in-file status here; it's updated by _update_file_progress
    ...
    def _reset_file_progress(self, total: int, file_path: Path) -> None:
        ...
        if self.file_progress:
            self.file_progress.configure(mode="determinate", maximum=1, value=0)
        self.progress_file_var.set(f"Current file: {file_path}")

    def _update_file_progress(self, value: int, total: int) -> None:
        self._ensure_progress_popup()
        if self.file_progress:
            self.file_progress.configure(maximum=1, value=1)
        path_display = self.current_file_path if self.current_file_path else ""
        self.progress_file_var.set(f"Current file: {path_display} (done)")
    ...
    def _set_busy(self, busy: bool, message: str) -> None:
        ...
        if self.fight_box:
            self.fight_box.state(["disabled"] if busy else ["!disabled"])
        if self.reload_btn:
            self.reload_btn.state(["disabled"] if busy else ["!disabled"])
        if self.analyze_btn:
            self.analyze_btn.state(["disabled"] if busy else ["!disabled"])
        if self.clear_cache_btn:
            self.clear_cache_btn.state(["disabled"] if busy else ["!disabled"])
        if self.display_names_btn:
            self.display_names_btn.state(["disabled"] if busy else ["!disabled"])
        if not busy and not self.verifying_players:
    ...
    def _ensure_progress_popup(self):
        if self.progress_popup and tk.Toplevel.winfo_exists(self.progress_popup):
            try:
                self._position_progress_popup()
                self.progress_popup.deiconify()
                self.progress_popup.state("normal")
        ...
        top = tk.Toplevel(self.frame)
        top.withdraw()
        top.overrideredirect(True) # Frameless
        top.configure(bg=self.colors["border"]) # Border color
        ...
        inner = ttk.Frame(top, style="Panel.TFrame", padding=2)
        inner.pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
        
        content = ttk.Frame(inner, style="Panel.TFrame", padding=20)
        content.pack(fill=tk.BOTH, expand=True)

        # Title
        ttk.Label(content, text="ANALYZING LOGS", font=("Segoe UI", 14, "bold"), style="Section.TLabel").pack(anchor=tk.CENTER, pady=(0, 15))

        # Progress Bar
        self.progress = ttk.Progressbar(content, mode="determinate", length=400, style="Futuristic.Horizontal.TProgressbar")
        self.progress.pack(pady=(0, 10))

        # Status Text
        lbl = ttk.Label(content, textvariable=self.progress_label_var, style="LabelMuted.TLabel", font=("Segoe UI", 10))
        lbl.pack(anchor=tk.CENTER)
        
        # File Text
        file_lbl = ttk.Label(content, textvariable=self.progress_file_var, style="LabelMuted.TLabel", font=("Segoe UI", 9), wraplength=400, justify=tk.CENTER)
        file_lbl.pack(anchor=tk.CENTER, pady=(5, 10))
        
        # Cancel Button
        ttk.Button(content, text="Cancel", command=self._cancel_load, style="Accent.TButton").pack(anchor=tk.CENTER)

        self.progress_popup = top
        self._position_progress_popup()
        ...
        top.after_idle(_show_centered)
        self._schedule_progress_keepalive()

    def _position_progress_popup(self) -> None:
        """Align the progress popup with the center of the app window or screen."""
        if not (self.progress_popup and tk.Toplevel.winfo_exists(self.progress_popup)):
            return

        width, height = self._progress_popup_size
        top = self.progress_popup
        ...
        try:
            self.app.update_idletasks()
            if self.app.winfo_viewable() and self.app.winfo_width() > 1 and self.app.winfo_height() > 1:
    ...
    def _center_window(self, window: tk.Toplevel, width: int, height: int) -> None:
        """Center a toplevel over the main frame; fallback to size set if positioning fails."""
        try:
            self.frame.update_idletasks()
    ...
    def _schedule_progress_keepalive(self):
        """Periodically re-show/lift the progress popup so it doesn't vanish when the app loses focus."""
        if not (self.progress_popup and tk.Toplevel.winfo_exists(self.progress_popup)):
            return
        try:
            state = self.progress_popup.state()
            ...
            pass
        if self.loading or self.verifying_players:
            # Keep nudging while work is active
            self.progress_popup.after(800, self._schedule_progress_keepalive)

    def _bind_app_visibility_events(self) -> None:
        """Hook window visibility/focus events once so the loader stays in sync."""
        if self._app_visibility_bound:
            return
        self._app_visibility_bound = True
        self.app.bind("<Map>", self._handle_app_visibility_event, add="+")
        self.app.bind("<FocusIn>", self._handle_app_visibility_event, add="+")

    def _handle_app_visibility_event(self, _event=None) -> None:
        """Restore the progress popup when the app becomes visible again."""
        if not (self.loading or self.verifying_players):
            return
        self._ensure_progress_popup()
    ...
    def _show_initial_popup(self) -> None:
        ...
        try:
            ...
            pw = self.frame.winfo_width() or 320
            ph = self.frame.winfo_height() or 180
            top.geometry(f"{pw}x{ph}+{px}+{py}")
            top.configure(bg=self.colors["bg"])

            if self.progress_popup and tk.Toplevel.winfo_exists(self.progress_popup):
                top.lower(self.progress_popup)
                try:
            ...
            # Overlay content
            container = ttk.Frame(top, style="Panel.TFrame", padding=20)
            container.place(relx=0.5, rely=0.5, anchor="center")
            ttk.Label(container, text="Loading…", style="Section.TLabel", anchor="center").pack()

            self._initial_popup = top
            self._initial_popup_shown = True
            self._initial_popup_pending = False
            top.after(5000, lambda: self._close_initial_popup())
        except Exception:
            self._initial_popup = None
            self._initial_popup_pending = False
    ...
    def _ensure_players_checked_then_render(self):
        ...
        self._ensure_progress_popup()
        self.progress_label_var.set("Verifying player identities...")
        self.progress["maximum"] = len(unknown_names)
    ...
    def _update_check_progress(self, name, value):
        if not self.progress:
            return
        self.progress_label_var.set(f"Checking API for: {name}")
        self.progress["value"] = value
